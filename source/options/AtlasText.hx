...
class AtlasText extends FlxTypedSpriteGroup<AtlasChar>
{
	...
	public var lastCharacter:AtlasChar = null;
	public var firstCharacter:AtlasChar = null;
		...
		for (i in 0...splitValues.length)
		{
			switch(splitValues[i])
			{
				...
				case char:
				{
					...
					add(charSprite);
					lastCharacter = charSprite;

					if (firstCharacter == null)
						firstCharacter = charSprite;
...
private class AtlasFontData
{
	...
	public function new (name:AtlasFont)
	{
		...
		trace("Add Text Letter From Font: " + name.getName().toLowerCase());
	}
}
