...
class FlxRuntimeShader extends FlxShader
{
	...
	private var __glslVersion:Int = 120;
	 ...
	 */
	public function new(fragmentSource:String = null, vertexSource:String = null, glsl_version:Int = 120):Void
	{
		__glslVersion = glsl_version;
		...
		if (vertexSource != null) {
			...
			trace('Loading default vertex fragment source...');
	...
	private function __getPrecisionHeaders():String
		return #if (js && html5)
	...
	private override function __initGL():Void
	{
		...
		if (__context != null && program == null)
		{
			...
			var precisionHeaders = __getPrecisionHeaders();
			var versionHeader = '#version ' + __glslVersion + '\n';

			var vertex = StringTools.replace(glVertexSource, "#pragma precision", precisionHeaders).replace("#pragma version", versionHeader);
			var fragment = StringTools.replace(glFragmentSource, "#pragma precision", precisionHeaders).replace("#pragma version", versionHeader);
		...
		#if (FLX_DRAW_QUADS && SHADER_TOY)
		data.iTime.value = [0.0];
		data.iTimeDelta.value = [0.0];
		data.iMouse.value = [0.0, 0.0, 0.0, 0.0];

		FlxG.signals.postUpdate.add(function() {
			data.iTime.value[0] += FlxG.elapsed;
			data.iTimeDelta.value[0] = FlxG.elapsed;
			#if FLX_MOUSE
			update_iMouse(FlxG.mouse);
			#end
		});
		#end
	}

	#if (FLX_MOUSE && FLX_DRAW_QUADS && SHADER_TOY)
	private function update_iMouse(mouse:FlxMouse = null) {
		if (mouse == null) return;

		mouse.getPosition(mousePosition);

		data.iMouse.value[0] = mousePosition.x;

		data.iMouse.value[1] = data.iResolution.value[1] - mousePosition.y;

		data.iMouse.value[2] = 0.0;
		data.iMouse.value[3] = 0.0;

		if (mouse.pressed){
			data.iMouse.value[2] = mousePosition.x;
			data.iMouse.value[3] = data.iResolution.value[1] - mousePosition.y;
		}
	}
	#end
