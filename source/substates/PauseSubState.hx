...
class PauseSubState extends MusicBeatSubstate
{
	...
	public function new(x:Float, y:Float)
	{
		...
		if (!difficultyChoices.contains('BACK'))
	...
	override function update(elapsed:Float)
	{
		...
		if (accepted)
		{
			...
			switch (daSelected)
			{
				...
				default:
					if (curMenu == 'diffSelect') {
						var diffShit:String = '-' + daSelected;

						if (daSelected.toLowerCase() == '-normal')
							diffShit = '';

						PlayState.SONG = Song.loadFromJson(Highscore.formatSong(PlayState.SONG.song.toLowerCase(), diffShit),
